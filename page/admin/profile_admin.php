    ...
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($error)) {
            ...
            if (!empty($new_password)) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE users 
                    SET nama = ?, email = ?, password = ?, foto = ?, updated_at = NOW()
                    WHERE id = ?
                ");
                $result = $stmt->execute([$nama, $email, $hashed_password, $foto_filename, $user_id]);
            } else {
                $stmt = $pdo->prepare("
                    UPDATE users 
                    SET nama = ?, email = ?, foto = ?, updated_at = NOW()
                    WHERE id = ?
                ");
                $result = $stmt->execute([$nama, $email, $foto_filename, $user_id]);
            }
...
<head>
    ...
    <title>Profile Admin - PERPUSTAKAAN DYZEN</title>
...
<body>
    ...
    <div class="main-content">
        <header class="bg-white rounded-lg shadow-sm p-6 mb-6">
            <div>
                <h1 class="text-2xl font-playfair font-bold text-primary">Profile Admin</h1>
        ...
        <div class="grid lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-lg shadow-sm p-6 text-center">
                    <?php if (!empty($admin['foto']) && file_exists('../../uploads/covers/' . $admin['foto'])): ?>
                        <img src="../../uploads/covers/<?= e($admin['foto']) ?>" class="profile-photo mx-auto mb-4" alt="Profile Photo">
                    <?php else: ?>
                        <div class="profile-photo mx-auto mb-4 bg-gradient-to-br from-primary to-secondary flex items-center justify-center">
                            <span class="text-white text-5xl font-bold"><?= strtoupper(substr($admin['nama'], 0, 1)) ?></span>
                        </div>
                    <?php endif; ?>
                    
                    <h3 class="font-bold text-xl"><?= e($admin['nama']) ?></h3>
                    <p class="text-text-light">Administrator</p>
                    <div class="mt-4 p-3 bg-light rounded-lg">
                        <p class="text-sm text-text">Member sejak: <?= date('d F Y', strtotime($admin['created_at'])) ?></p>
                        <p class="text-sm text-text">Terakhir login: <?= $admin['last_login'] ? date('d/m/Y H:i', strtotime($admin['last_login'])) : '-' ?></p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <form method="POST" action="" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="flex justify-end space-x-3 pt-4">
                            <a href="index_admin.php" class="btn-outline">Batal</a>
